...
def get_total_duration_per_language(base_folder):
    ...
    try:
        # Iterate through subfolders where folder names represent languages
                    ...
                    for file_name in files:
                        if file_name.lower().endswith(('.wav', '.mp3', '.flac', '.aac', '.ogg', '.m4a')):
                            ...
                            if duration:
                                ...
                                total_seconds_per_language[language_folder] += total_seconds
                                audio_files_per_language[language_folder].append((file_path, hours, minutes, seconds))

        # Convert total seconds per language to hours, minutes, and seconds
        total_durations = {}
        for language, total_seconds in total_seconds_per_language.items():
            hours = int(total_seconds // 3600)
            minutes = int((total_seconds % 3600) // 60)
            seconds = int(total_seconds % 60)
            total_durations[language] = (hours, minutes, seconds)
...
def export_counts_to_tsv(total_durations, audio_files_per_language):
    ...
    output_file = os.path.join(current_directory, "audio_counts.tsv")
    ...
    try:
        with open(output_file, 'w', newline='', encoding='utf-8') as tsv_file:
            ...
            for language, files in audio_files_per_language.items():
                ...
                writer.writerow([language, file_count, total_duration_str])
...
if __name__ == "__main__":
    ...
    if result:
        ...
        for language, (hours, minutes, seconds) in total_durations.items():
